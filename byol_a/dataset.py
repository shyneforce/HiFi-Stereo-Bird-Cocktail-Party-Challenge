...
class DataInLMSOutDataset(Dataset):
    """Data in, log-mel spectrogram out, dataset class.
    ...
    def __init__(self, cfg, audio_files, labels, tfms, use_librosa=False):
        ...
        assert (labels is None) or (
                    len(audio_files) == len(labels)), 'The number of audio files and labels has to be the same.'
        super().__init__()
    ...
    def __getitem__(self, idx):
        # load single channel .wav audio
        list_wav = self.files[idx]
        wav = torch.FloatTensor(list_wav)
